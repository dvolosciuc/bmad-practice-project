  ...
  return (
    <div className="bg-ev-surface-2 rounded-xl p-6 border border-transparent border-l-2 border-l-ev-accent hover:border-ev-accent/30 transition-colors">
      ...
      <table className="w-full text-sm mb-4">
        {(() => {
          const hasDC = operator.tariffs?.some((t) => t.dcFromMDL != null) ?? false
          return (
            <>
              <thead>
                <tr className="text-ev-muted text-[13px] uppercase tracking-wider">
                  ...
                  {hasDC && <th className="text-right py-1">DC (MDL/kWh)</th>}
                </tr>
              </thead>
              <tbody>
                {operator.tariffs?.map((tariff) => (
                  <tr key={tariff.region} className="border-t border-ev-surface-2">
                    ...
                    {hasDC && <td className="py-2 text-right text-ev-accent font-medium">{tariff.dcFromMDL ?? '—'}</td>}
                  </tr>
                ))}
              </tbody>
            </>
