...
function PriceBlock({ type, value, sublabel }: { type: 'ac' | 'dc'; value: number; sublabel?: string }) {
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-xl p-4 flex flex-col gap-1 ${
        isAc ? 'bg-ev-accent/8 border border-ev-accent/20' : 'bg-violet-500/8 border border-violet-500/20'
      }`}
    >
      ...
      {/* Label */}
      <div
        className={`relative flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-[0.12em] ${
      ...
      {/* Big price number */}
      <div
        className={`relative text-3xl font-black tabular-nums leading-none ${
          isAc ? 'text-ev-accent' : 'text-violet-400'
        }`}
      >
        {value}
  ...
  return (
    <div className="flex gap-2 flex-wrap mt-5 pt-4 border-t border-white/5">
      ...
      {operator.playStoreUrl && (
        ...
        >
          <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M3.18 23.76c.3.17.64.22.99.14l13.2-7.62-2.82-2.82-11.37 10.3zm-1.07-20.7C2.04 3.3 2 3.57 2 3.86v16.28c0 .29.04.56.11.8l.06.06 9.12-9.12v-.2L2.17 2.99l-.06.07zm18.54 9.09l-2.64-1.53-3.06 3.06 3.06 3.06 2.66-1.54c.76-.44.76-1.61-.02-2.05zM4.17.28l13.2 7.62-2.82 2.82L3.18.42c.3-.17.67-.2.99-.14z" />
          </svg>
          {androidLabel}
        </a>
      )}
      ...
      <div className="p-6">
        ...
        <div className="flex flex-col gap-5">
          {operator.tariffs?.map((tariff) => (
            <TariffSection key={tariff.region} tariff={tariff} regionLabel={t(`charging.region.${tariff.region}`)} />
          ))}
        </div>

        <StoreLinks operator={operator} iosLabel={t('charging.appStore')} androidLabel={t('charging.playStore')} />
      </div>
    </div>
  )
}
